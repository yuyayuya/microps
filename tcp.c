...
struct tcp_pcb {
    ...
    struct {
        uint32_t nxt; // 次に送信すべきシーケンス番号
        uint32_t una; // まだACKの確認が得られていないシーケンス番号
        uint16_t wnd;
        uint16_t up;
        uint32_t wl1;
        uint32_t wl2;
    } snd;
    uint32_t iss; // シーケンス番号
    struct {
        uint32_t nxt; // 次に受信すべきシーケンス番号
        uint16_t wnd;
        uint16_t up;
    } rcv;
...
tcp_retransmit_queue_cleanup(struct tcp_pcb *pcb)
{
    ...
    while (1) {
        ...
        debugf("remove, seq=%u, flag=%s, len=%u", entry->seq, tcp_flg_ntoa(entry->flg), entry->len);
        memory_free(entry);
    }
    return;
}
...
tcp_retransmit_queue_emit(void *arg, void *data)
{
    ...
    timersub(&now, &entry->first, &diff); // 初回送信からの経過時間を計算
...
tcp_output(struct tcp_pcb *pcb, uint8_t flg, uint8_t *data, size_t len)
{
    ...
    if (TCP_FLG_ISSET(flg, TCP_FLG_SYN | TCP_FLG_FIN) || len) { // シーケンス番号を消費するセグメントだけ再送キューへ格納する(単純なACKやRSTは対象外)
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    case TCP_PCB_STATE_SYN_SENT:
         ...
         */
        if (TCP_FLG_ISSET(flags, TCP_FLG_ACK)) {
            if (seg->ack <= pcb->iss || seg->ack > pcb->snd.nxt) { // 送信していないシーケンス番号に対するACKだったらRSTを返す
                tcp_output_segment(seg->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
                return;
            }
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (acceptable) {
                pcb->snd.una = seg->ack; // 未確認のシーケンス番号を更新(ACKの値は「次に受信すべきシーケンス番号」を示すのでACKの値と同一のシーケンス番号の確認は取れていない)
                tcp_retransmit_queue_cleanup(pcb); // 再送キューからACKによって到達が確認できているTCPセグメントを削除
            }
            if (pcb->snd.una > pcb->iss) { // 初期シーケンス番号(つまりSYN)に対するACKが得られていた場合の処理
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    debugf("%s:%d => %s:%d, len=%zu (payload=%u)",
